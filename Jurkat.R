...
## Selected samples to be extracted ('Jurkat')
...
protein_coding_ind = which(gene_biotype == 'protein_coding') #protein-coding index
...
non_zero_genes = apply(expression, 1, function(row) all(row >0.001))
expression = expression[non_zero_genes,]

## Select the most variable genes:
mostvariablegenes <- names(tail(sort(apply(expression,1,var)), n=10000))
expression_var <- expression[which(rownames(expression) %in% mostvariablegenes),]

## Transcription Factors used in analysis (AnimalTFDB 3.0)
TF = read.csv('TF.csv', header = FALSE)
Factors = as.vector(TF$V1)
Factors = Factors[Factors %in% rownames(expression_var)]

## GENIE3 
set.seed(123) 
weightMatrix <-GENIE3(exprMatrix=expression_var, regulators = Factors, targets = NULL, 
...
write.table(weightMatrix, file="weightMatrix_Jurkat.csv")
write.table(linkList, file="linkList_Jurkat.csv")
linkList <- read.table("linkList_Jurkat.csv", quote="\"", comment.char="")
linkList$V1 <- NULL
...
write.table(head(linkList_mostvar,10000), file = "NETWORK_jurkat_10.tsv", sep = "\t", quote = F, row.names = F, col.names = F)
write.table(head(linkList_mostvar,50000), file = "NETWORK_jurkat_50.tsv", sep = "\t", quote = F, row.names = F, col.names = F)

## VIPER

nwfile10 <- "NETWORK_jurkat_10.tsv"
nwfile50 <- "NETWORK_jurkat_50.tsv"

regulon10 <-  aracne2regulon(afile=nwfile10, eset = expression_var, gene = FALSE, format = "3col", verbose = T)
regulon50 <-  aracne2regulon(afile=nwfile50, eset = expression_var, gene = FALSE, format = "3col", verbose = T)
...
write.table(NES, file = "NES_Jurkat.tsv", sep = "\t", quote = F, row.names = F, col.names = T)
write.table(Pval, file = "Pval_Jurkat.tsv", sep = "\t", quote = F, row.names = F, col.names = T)

## Heatmap

NES_Jurkat <- read.delim("NES_Jurkat.tsv")
Pval_Jurkat <- read.delim("Pval_Jurkat.tsv")


df <- NES_Jurkat[,-1]
rownames(df) <- NES_Jurkat$TF
df[is.na(df)] <- 0

df2 <- Pval_Jurkat[,-1]
rownames(df2) <- Pval_Jurkat$TF
df2[is.na(df2)] <- 100
...
pheatmap(as.matrix(sign_genes_nes), display_numbers = sign_genes, 
         fontsize_number = 20,
         scale = "row",
         angle_col = 45, fontsize_row=15,fontsize_col = 16,treeheight_row = 82, treeheight_col = 42, 
